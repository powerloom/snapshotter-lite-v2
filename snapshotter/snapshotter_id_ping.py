...
async def main():
    ...
    try:
        ...
        if slot_id_snapshotter_addr == Web3.to_checksum_address(snapshotter_address):
            ...
            print('🔑 Using existing P2P Ed25519 private key for DSV devnet...')

            # Use existing P2P private key
            await use_existing_p2p_key()

            print('🎉 Slot ID validation and P2P key setup completed successfully!')
        else:
...
async def use_existing_p2p_key():
    ...
    try:
        ...
        if not existing_private_key_hex:
            print('❌ LOCAL_COLLECTOR_PRIVATE_KEY environment variable not found!')
        ...
        print(f'🔑 Using existing P2P private key (128 hex chars): {existing_private_key_hex[:16]}...{existing_private_key_hex[-16:]}')
        print('   Key was generated by init_docker.sh container initialization')
        print('🔗 P2P key ready for use by local collector')

    except Exception as e:
        print(f'❌ Failed to use existing P2P private key: {e}')
